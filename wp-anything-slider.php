...
add_filter('the_content','wpanything_show_filter');

function wpanything_show_filter($content)
{
	return 	preg_replace_callback('/\[WP-ANYTHING-SLIDER=(.*?)\]/sim','wpanything_show_filter_callback',$content);
}

function wpanything_show_filter_callback($matches) 
{
	global $wpdb;
	$setting = $matches[1];
	$wpcycle = "";
...
add_action('init', 'wpanything_add_javascript_files');
